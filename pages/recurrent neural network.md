...
- an RNN is a [[neural network]] where the output of a node can influence the input of that same node. it is bidirectional, unlike a basic [[feedforward neural network]]
	...
	- said another way, they compute a sequence of hidden states $h_i$, each a function of the prior state $h_{i-1}$ and the input
...
- [[attention (ml)]] largely swept away RNNs, due to being more parallelizable, and having better empirical results.