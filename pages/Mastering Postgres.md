...
- # Advanced Data Types
	- ## Intervals
		- these represent a duration or a period of time. you might want them for tasks like determining if appointments overlap
		- for example:
			- ```
			  SELECT "1 year 2 months 5 day"::interval;
			  ```
		- you can configure the interval style, for example, to ISO 860 format: ```P1Y2M5DT0H0M0S```
	- ## Serial
		- it's not exactly real! it creates a sequence of integers
		- it can develop gaps in the sequence. for example, if two conflicting transactions are open, and each increments the sequences, then only one commits- the other one will leave a gap.
		- probably don't use it for primary keys. if you have to, use `bigserial` for more headroom.
		- you can use a `RETURNING` clause in Postgres to get back autogenerated stuff like IDs and serials!
	- ## Sequence
		- you can configure several parameters:
			- ```
			  CREATE SEQUENCE foo AS bigint INCREMENT 1 START 1 MINVALUE 1
			    MAXVALUE 100 CACHE 1;
			  ```
		- you can select the `NEXTVAL()` and `CURRVAL()` from a sequence. `CURRVAL()` actually gives you _the last value `NEXTVAL()` returned_.
		- you can run `SETVAL()` to reset a sequence
	- ## Identity
		- use these as your primary keys! `bigint` or `uuid` are good choices
		- ```
		  id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY
		  ```
		- `BY DEFAULT` is a little looser than `ALWAYS`. `ALWAYS` will give you an error if you try to set the id manually. `BY DEFAULT` will let you set it. but you'll often run into weirdness and have to override the system value, so probably avoid it!
	- ## Network and MAC Addresses
		- using an `inet`, `cidr`, `macaddr`, or `macaddr8` is much more compact than using `text`!
	- ## JSON
		- if you have a rigid, well-defined schema, consider breaking it into columns instead of using a big blob o' JSON
		- consider:
			- is there a rigid schema
